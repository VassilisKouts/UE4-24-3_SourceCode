				...
				if (Function && Function->GetNamedParameterOfType(ParameterInfo, Parameter, &ParameterOwner))
				{
					if (Parameter)
					{
						...
						if (OutValue)
						{
							...
							if (AtlasParameter && AtlasParameter->ParameterName == ParameterInfo.Name)
							{
								Curve = TSoftObjectPtr<UCurveLinearColor>(FSoftObjectPath(AtlasParameter->Curve->GetPathName()));
								Atlas = TSoftObjectPtr<UCurveLinearColorAtlas>(FSoftObjectPath(AtlasParameter->Atlas->GetPathName()));
								return true;
...
void UMaterial::GetQualityLevelNodeUsage(TArray<bool, TInlineAllocator<EMaterialQualityLevel::Num> >& OutQualityLevelsUsed)
{
	...
	for (bool Value : OutQualityLevelsUsed)
	{
		if (Value)
		{
...
void UMaterial::PostLoad()
{
	...
	DoMaterialAttributeReorder(&LutMap, UE4Ver);
#endif // WITH_EDITORONLY_DATA
...
int32 UMaterial::CompilePropertyEx( FMaterialCompiler* Compiler, const FGuid& AttributeID )
{
	...
	switch (Property)
	{
		...
		case MP_LutMap:					return LutMap.CompileWithDefault(Compiler, Property);
	...
	bool bUsesShadingModelFromMaterialExpression)
{
	...
	case MP_SubsurfaceColor:
		Active = ShadingModels.HasAnyShadingModel({ MSM_Subsurface, MSM_PreintegratedSkin, MSM_TwoSidedFoliage, MSM_Cloth, MSM_SGSSS });//add SG_SSS shading model 自定义材质引脚SubsurfaceColor
		break;
	case MP_CustomData0:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ClearCoat, MSM_Hair, MSM_Cloth, MSM_Eye, MSM_SGSSS });//add SG_SSS shading model 自定义材质引脚CustomData0
		break;
	case MP_CustomData1:
	...
	case MP_ShadingModel:
        ...
        break;
	case MP_LutMap:
		Active = ShadingModels.HasAnyShadingModel({ MSM_MobilePreintegratedSkin });
		break;
	case MP_MaterialAttributes:
	default:
		Active = true;
